...
public class MazeLoader {
	...
	private static final String ROBOT_CLASSNAME = "ROBOT_CLASSNAME";
	...
	private void processLine(MazeBean maze, String line) {
		...
		case ROBOT_CLASSNAME:
			maze.setRobotClassname(fields[1]);
			break;
	...
	private void addMoviments(MazeBean maze, int lineIndex, int columnIndex, String[] values) {
		...
		if (maze.getMap()[lineIndex][columnIndex] == null) {
			maze.getMap()[lineIndex][columnIndex] = new ArrayList<ActionType>();
		}

		ActionType moviment = null, nearMoviment = null;

		for (int i = 0; i < values.length; i++) {
			moviment = ActionType.parse(values[i]);
			...
			case NORTH:
				nearLineIndex--;
				nearMoviment = ActionType.SOUTH;
				break;

			case WEST:
				nearColumnIndex--;
				nearMoviment = ActionType.EAST;
				break;

			case SOUTH:
				nearLineIndex++;
				nearMoviment = ActionType.NORTH;
				break;

			case EAST:
				nearColumnIndex++;
				nearMoviment = ActionType.WEST;
				break;

			case STAY:
				nearMoviment = ActionType.STAY;
				break;
			}
			...
			if (maze.getMap()[nearLineIndex][nearColumnIndex] == null) {
				maze.getMap()[nearLineIndex][nearColumnIndex] = new ArrayList<ActionType>();
			}
	...
	private boolean ignoreMoviment(MazeBean maze, int lineIndex, int columnIndex, ActionType moviment) {
		if ((moviment == ActionType.NORTH) && (lineIndex == 0)) {
			return true;
		}

		if ((moviment == ActionType.WEST) && (columnIndex == 0)) {
			return true;
		}

		if ((moviment == ActionType.SOUTH) && (lineIndex == maze.getMazeHeight() - 1)) {
			return true;
		}

		if ((moviment == ActionType.EAST) && (lineIndex == maze.getMazeWidth() - 1)) {
			return true;
		}
